          ...
          {speakers.map(speaker => (
            <div key={speaker.id} className="w-full max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                <div className="flex flex-col items-center pb-10">
                     <div className="relative my-4 h-24 w-24">
                     <Image className="rounded-full shadow-lg" src={speaker.image} alt={speaker.name} layout="fill" objectFit='cover'/>
                     </div>
                   
                    <h5 className="mb-1 text-xl font-medium text-gray-900 dark:text-white">{speaker.name}</h5>
                    <span className="text-sm text-gray-500 dark:text-gray-400">{speaker.expertise}</span>
                    <div className="flex mt-4 space-x-3 md:mt-6">
                        <a href="#" className="inline-flex items-center py-2 px-4 text-sm font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Website</a>
